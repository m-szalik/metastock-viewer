...
public class Reader {
	 ...
	 * @see com.mac.verec.trading.turtle.TurtleReader
	 */
	protected Quote createQuote(int no, NumberDate	date 
				,	float	open 
				,	float	high 
				,	float	low
				,	float	close
	...
	public void onRecordReaded(MasterFileRecord r) {
	}
	...
	private Quote[]
	importQuotes(MasterFileRecord r, AtomicInteger norec, String dataPath) {
		...
		try {
			...
			for (int i = r.recordLength ; i < length ; i += r.recordLength, ++quoteIndex) {
				...
				Quote q = createQuote(quoteIndex, nd,	row[1]
													,	row[2]
													,	row[3]
													,	row[4]
													,	row[5]
